			...
			while (true) {
				try {
					if (client.getZookeeperClient().blockUntilConnectedOrTimedOut()) {
						...
						if(!znodeApi.checkExists(client, Constants.ROOT+Constants.EJOB_SERVER_NODE_ROOT+Constants.EJOB_SERVER_NODE_SLAVE+Constants.PATH_SEPARATOR + localHost.getIp()+ Constants.EJOB_SERVER_NODE_SLAVE_STATUS)){
							client.create().creatingParentsIfNeeded().withMode(CreateMode.EPHEMERAL)
							.forPath(Constants.ROOT+Constants.EJOB_SERVER_NODE_ROOT+Constants.EJOB_SERVER_NODE_SLAVE+Constants.PATH_SEPARATOR + localHost.getIp()+ Constants.EJOB_SERVER_NODE_SLAVE_STATUS, Constants.READY.getBytes("UTF-8"));
						}
						break;
					}
