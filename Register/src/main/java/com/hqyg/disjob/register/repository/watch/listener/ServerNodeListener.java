...
import com.hqyg.disjob.rpc.client.proxy.RpcClientCache;
import com.hqyg.disjob.rpc.client.proxy.ServerLinkedService;
 ...
 * 
 *  Description:
 *  路径:/ejob/rpc/group/servicename/provide
...
public class ServerNodeListener implements PathChildrenCacheListener{
    ...
    public void childEvent(CuratorFramework client, PathChildrenCacheEvent event) throws Exception {
       if(event.getType() == org.apache.curator.framework.recipes.cache.PathChildrenCacheEvent.Type.CHILD_ADDED){
           ...
           String path = event.getData().getPath();//->path:/ejob/rpc/group/jobname/providers/ip:port
		   ...
		   if (StringUtils.isNotEmpty(path)) {
				...
				if (event.getType() == org.apache.curator.framework.recipes.cache.PathChildrenCacheEvent.Type.CHILD_ADDED) {
					...
					if (uri != null&& uri.length >= 5&& uri[5].equalsIgnoreCase(Constants.EJOB_PROVIDERS)) {
						String jobPath = path.substring(0,path.indexOf(Constants.EJOB_PROVIDERS) - 1).replace("rpc", "job");//-> ejob/job/group/jobname/
    ...
    * 检测是否需要立即触发job.
    * @param client
    * @param path /ejob/rpc/group/jobname/providers/ip:port
    */
